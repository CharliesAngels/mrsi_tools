cluster_stem=/home/fs0/leahm/scratch/Joystick_Cereb-MRS/Data/
SUITDIR=/Users/CN/Documents/Joystick_Cereb_MRS/MRSIanalysis/spm12/toolbox/suit
# ------------------------------------------------------------------------------
# Register all voxinVOI to T1
for i in ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/T1.nii; do
    cd ${i%T1.nii}
...
ls ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/voxInVOI_T1.nii.gz
# In matlab: Run process_all_suit.m step 3: reslice into SUIT space using DARTEL (line 33 - 77)
ls ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/voxInVOI_T1_sstd.nii
...
ls ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/voxInVOI_T1_sstd.nii* | wc
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
...
ls ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/${overlay_image}.nii* > ${cluster_stem}/bidsdir/code/${inputlist}.txt
# Remove subject M03 (sub-03) and subject F01 (sub-12)
sed -i.bak '/sub-03/d' ${cluster_stem}/bidsdir/code/${inputlist}.txt
sed -i.bak '/sub-12/d' ${cluster_stem}/bidsdir/code/${inputlist}.txt
# ------------------------------------------------------------------------------
fslmaths $SUITDIR/atlasesSUIT/SUIT.nii -add 1 -bin ${cluster_stem}/bidsdir/derivatives/${output}
# ------------------------------------------------------------------------------

# for i in `ls ${cluster_stem}/bidsdir/derivatives/*/ses-*/mrsi/base/qa/QA_GABA_sstd.nii*`; do
while read i; do
    ...
    if ${Add_mask}; then
        ...
        fslmaths ${cluster_stem}/bidsdir/derivatives/${output} \
         -add $i \
         ${cluster_stem}/bidsdir/derivatives/${output}

     fi
done <${cluster_stem}/bidsdir/code/${inputlist}.txt
cd ${cluster_stem}

fsleyes -vl 91 41 44 $SUITDIR/atlasesSUIT/SUIT.nii ${cluster_stem}/bidsdir/derivatives/${output} -dr 1 30 -cm cool &
fsleyes -vl 100 45 45 $FSLDIR/data/standard/MNI152_T1_1mm_brain.nii.gz ${cluster_stem}/bidsdir/derivatives/${output} -dr 1 30 -cm cool &
